...
```bash
docker build -t oricha/activemq .
docker push oricha/activemq
```

or for arm based architectures

```bash
docker build -f Dockerfile.arm32v7 -t oricha/activemq:armv7 .
docker push oricha/activemq:armv7
```
...
```bash
docker run --name='activemq' -it --rm -P oricha/activemq
```
